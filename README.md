...
**加强方向**：自动语言识别（linguist）；注入 Hash 用于去重；加 `tenant_id` 支持未来 SaaS 多租户。
...
**做什么**：根据 `object_type` 路由到对应子流程。`code` → 代码拆解链，`api` → API 拆解链，`db` → 数据库拆解链，`container` → 镜像拆解链。
...
**验收门禁**：
- 状态 = `running` → 等待（轮询或 webhook 回调）
- 状态 = `done` → 直接返回缓存结果，流程终止

**加强方向**：加 TTL（超时后重新拆）；加版本号（源码变了重新拆）。
...
**加强方向**：按文件大小动态分批；与消息队列联动（RabbitMQ / Redis Queue）。

---

### 第三组：拆解引擎（禁止 AI 介入）

> ⚠️ 本组节点严禁 AI 参与。扫描结果必须 100% 可信。
...
**加强方向**：多语言并行分析；同时输出 Graphviz（可视化）+ JSON（机器消费）。
...
**加强方向**：写成独立微服务，任何流水线都能调；同步写图数据库（Neo4j）。
...
```json
{
  "package": "vast-core-auth-v1.tar.gz",
  "readme": {
    "what": "用户认证模块",
    "how_to_register": "from .auth import router",
    "env_vars": ["JWT_SECRET", "TOKEN_EXPIRE"],
    "interface": "GET /auth/token, POST /auth/login"
  }
}
```
